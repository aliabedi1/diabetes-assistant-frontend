...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-slate-950 text-white dark:bg-slate-950 dark:text-white">
      ...
      <section className="relative mx-auto max-w-7xl px-6 py-8">
        <nav className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <ThemeToggle />
            <LanguageToggle />
            <Link to="/login" className="rounded-2xl px-5 py-3 text-sm font-semibold text-slate-200 hover:bg-white/10">{t("nav.login")}</Link>
            <Link to="/register" className="rounded-2xl bg-white px-5 py-3 text-sm font-semibold text-slate-950">{t("nav.startFree")}</Link>
          </div>
        </nav>
        <div className="grid min-h-[calc(100vh-120px)] items-center gap-12 py-16 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            ...
            <div className="mt-10 flex flex-wrap gap-4">
              <Button as={Link} to="/register">{t("home.getStarted")}</Button>
              <Button as={Link} to="/login" variant="secondary">{t("home.openDashboard")}</Button>
            </div>
          </div>
          <Card className="bg-white/95 text-slate-950 dark:bg-slate-800/95 dark:text-white">
